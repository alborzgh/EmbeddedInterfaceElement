...
Matrix       EmbeddedBeamInterface::m_InterfaceStiffness(EBI_NUM_DOF, EBI_NUM_DOF);
Vector       EmbeddedBeamInterface::m_InterfaceForces(EBI_NUM_DOF);
...
EmbeddedBeamInterface::EmbeddedBeamInterface(int tag, int beamTag, int solidTag, int crdTransfTag,
    ...
    double radius, double area) : Element(tag, ELE_TAG_EmbeddedBeamInterface),
    externalNodes(10),
    ...
    OPS_Stream &s)
{
    if (strcmp(argv[0], "locationBeam") == 0 || strcmp(argv[0], "locBeam") == 0) {
        ...
        opserr << "EmbeddedBeamInterface Recorder, " << argv[0] << "is an unknown recorder request"
...
EmbeddedBeamInterface::getResponse(int responseID, Information &eleInformation)
{
    if (responseID == 1) { // location
        ...
        opserr << "EmbeddedBeamInterface, tag = " << this->getTag()
...
EmbeddedBeamInterface::setDomain(Domain *theDomain)
{
    ...
    {
        opserr << "EmbeddedBeamInterface::setDomain(): Error initializing coordinate transformation";
        return;
    }

    m_beam_length = crdTransf->getInitialLength();
    if (m_beam_length < 1.0e-12) {
        opserr << "FATAL ERROR EmbeddedBeamInterface (tag: " << this->getTag() << ") : "
...
int EmbeddedBeamInterface::updateShapeFuncs()
{
...
Vector
EmbeddedBeamInterface::CrossProduct(const Vector &V1, const Vector &V2)
{
    Vector V3(3);
...
Vector
EmbeddedBeamInterface::Geta1(void) 
{
    Vector a1(3);
    int i;
...
Vector
EmbeddedBeamInterface::Getb1(void) 
{
    Vector b1(3);
    int i;
...
void
EmbeddedBeamInterface::Setc1(Vector c1_vec) 
{
    mc1 = c1_vec;
...
Vector
EmbeddedBeamInterface::Getc1(void) {
    return mc1;
}


Matrix
EmbeddedBeamInterface::Transpose(int dim1, int dim2, const Matrix &M)
{
...
Matrix
EmbeddedBeamInterface::ComputeSkew(Vector th)
{
    Matrix skew_th(3, 3);
...
void
EmbeddedBeamInterface::ComputeBphiAndBu(Matrix &Bphi, Matrix &Bu)
{
    int i, j;
    Matrix dummy1(3, 3);
    Matrix dummy2(3, 3);
...
void
EmbeddedBeamInterface::UpdateTransforms(void)
{
...
void
EmbeddedBeamInterface::ComputeQc()
{
...
Matrix
EmbeddedBeamInterface::ExponentialMap(Vector th)
{
